...
!(function(){
	...
	$(".BigImgBox").on("mouseenter",function(){//鼠标移入大图时,遮罩层显示,大图显示
		pointer.css({"display":"block"});
		$(".MagnifyingGlass").css({"display":"block"});
	});
	$(".BigImgBox").on("mouseleave",function(){//鼠标移出大图时,遮罩层隐藏,大图隐藏
	...
	$(".ImgInfo").on("mouseenter",function(){//鼠标放到宝贝上时,停止轮播图
		clearInterval(timer);
		$(".MagnifyingGlass").find("img").attr("src",$(".BigImg").find("li").eq(i).find("img").attr("src"));//鼠标移动到大图里面时,大图的链接跟随其改变
	});
	...
	$(".SmallImg li").click(function(){//鼠标点击下面的图片时,跳转到指定的图片
		i = $(this).index();
		$(".MagnifyingGlass").find("img").attr("src",$(".BigImg").find("li").eq(i).find("img").attr("src"));//鼠标点击小图时,大图的链接跟随其改变.
	...
	//点击加入购物车按钮添加宝贝
	$(".GoodsBtn").find("button").eq(1).click(function(){//点击人气宝贝上购物车按钮时,阻止a链接的跳转,然后将该id和数量发送到服务器,且在发送前添加token头
	if($(".user ul li a").eq(0).text() == "登录"){
		alert("请先登录");
		return false;
	};
	...
	$.ajax({
		...
		success : function(data){//添加成功后,根据返回的message来判断
			if(data.message == "添加到购物车成功"){//第一次添加且成功后将本地数据添加到模板然后添加到购物车中
				...
				objdata.goods_name = "小丑5臂烛台";//宝贝名称
				objdata.goods_number = obj.number;//添加的数量
				objdata.goods_price = "2220.00";//宝贝的价格
